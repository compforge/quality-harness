...
runner never touched — a coverage gap, not a capacity result), and within-arm_run
...
IDLE_SHARE = 0.05
#: within-arm_run growth above this share of the start value → growth flag
GROWTH_PCT = 10.0
...
def _usage_families(r: ArmRun) -> list[str]:
...
def _peaks(store: MetricStore, r: ArmRun, family: str) -> dict[str, float]:
...
def _slope(
    label: str, rs: list[ArmRun], store: MetricStore,
    ...
    for r in rs:
        ...
        if peak is not None:
            points.append((r.arm.load.peak_level, peak))
    fit = linfit(points)
    if fit is None:
        return []
    slope, intercept = fit
    top_level, top_peak = points[-1]
    # idle: the sweep barely moves this service — a runner-coverage gap, not headroom
...
def _growth(label: str, top: ArmRun, family: str, unit: str) -> list[AnalysisNote]:
    """Within-arm_run first→last growth at the top level (fast-climb smell; a slow leak
        ...
        if first > 0 and growth / first * 100 >= GROWTH_PCT:
            out.append(
                AnalysisNote(
                    "resource",
                    "flag",
                    f"[{label}] {sid} 单 arm_run 内增长 {growth:.0f}{unit}"
