...
from harness_toolbox.kube import KubernetesClient, KubernetesClientFactory, Options, PodRef
...
async def test_list_pods_projects_stable_state_and_sorts() -> None:
    ...
    client = KubernetesClient(KubernetesClientFactory(TEST_OPTIONS), api=api)
...
async def test_wait_ready_rejects_reused_name() -> None:
    client = KubernetesClient(
        KubernetesClientFactory(TEST_OPTIONS),
...
async def test_wait_ready_bounds_inflight_kubernetes_request() -> None:
    client = KubernetesClient(KubernetesClientFactory(TEST_OPTIONS), api=SlowReadCoreV1API())
...
def test_client_rejects_missing_scope_or_limits(options: Options) -> None:
    with pytest.raises(ValueError):
        KubernetesClient(KubernetesClientFactory(options), api=FakeCoreV1API())
...
async def test_exec_stdin_exit_status_and_uid_check(tmp_path):
    ...
    client = KubernetesClient(KubernetesClientFactory(TEST_OPTIONS, kubectl=str(script)), api=api)
...
async def test_root_disposal_drains_exec(tmp_path):
    ...
    script.chmod(0o755)
    client = KubernetesClient(
        KubernetesClientFactory(TEST_OPTIONS, kubectl=str(script)),
        api=FakeCoreV1API([pod("worker", "uid")]),
    )
...
async def test_in_cluster_api_and_exec_pin_the_same_identity(monkeypatch):
    ...
    client = KubernetesClient(KubernetesClientFactory(TEST_OPTIONS))
