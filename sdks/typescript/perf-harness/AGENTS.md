...
公开名词、调度语义和 `run.json` / `requests.jsonl / evaluations.json` 字段共同遵守 `spec/` 契约。

第一阶段只拥有负载调度、Runner 扩展、请求侧 Outcome、Window 归约和 raw/model 落盘。
...
1. 延迟按 dispatch cohort 归窗，吞吐按真实 dispatch/completion 事件时刻计数。
2. `Runner.fire` 只记录事实，`judge` 是唯一 verdict 权威。
3. 未发出的请求与被强制中断的在途请求不进入延迟分布。
4. `trace_id` 等跨平面对齐键放在 `Outcome.meta`，落盘时不得丢失。
5. 通用包不写业务 SSE 语义；业务 Plugin/consumer 实现 Runner。
6. Case/CaseSet 归 spec-case；Perf Experiment 只通过 `caseMix` 选择 id 和设置 weight。
7. Kernel 对齐：Outcome 是 request Observation；request、window、run 是不同 Unit grain，必须区分分析粒度。raw/model Run facts 可由不同 独立 Judge、SLO 和 analysis 组件离线复评，当前请求判定保存为独立 evaluations 表，不得因换判定口径重新发压；详见 `../../../docs/kernel.md#dataset-与反复评估`。
...
```bash
bun install --frozen-lockfile # 从 sdks/typescript workspace 安装
bun test
bun run typecheck
bun run build
```
...
- `../../../spec/perf-run-schema.yaml` / `perf-request-schema.yaml` — 落盘 schema
